#include <stdio.h>            
 ...
 #define VINPORT 2012          //default port number  
 //Note:  If no port is specified,the server uses the default given by PORT.  
 ...
 int main(int argc, char *argv[]) {  
  ...
  if( argc == 2 )       
      ...
      portnum = VINPORT; //if port number not given as argument then using default port  
      printf("\nPORT NO.:\t%d\n",portnum);  
   ...
   if(sockfd == -1){  
      printf("\nserver- socket() error");     // debugging  
      exit(1);  
   }else  
      printf("socket created\n");  
   if (setsockopt(sockfd, SOL_SOCKET, SO_REUSEADDR, &yes,sizeof(int)) == -1) {  
      printf("set sockopt error\n");     // debugging  
      exit(1);  
   }else printf("reusing the port\n");  
      //=binding socket=  
   if(bind(sockfd, (struct sockaddr *)&server_addr, sizeof(struct sockaddr))==-1){  
      ...
      printf("\nbinding success");  
 printf("\t\t\nPRESS CTRL+z TO VIEW ONLINE CLIENTS");  
   ...
   while(1){  
           ...
           if(recv(new_fd,username,sizeof(username),0)>0);  
             ...
             printf("\t\n%d->%s JOINED chatroom",new_fd,username);  
           ...
           send(sf2,buffer ,sizeof(buffer),0);  
              ...
              printf("\nserver got connection from %s & %d",inet_ntoa(client_addr.sin_addr),new_fd); // debugging  
 ...
 void *server(void * arguments){  
  ...
  addr      a;  
   ...
   while(1){  
      ...
      jmp:     printf("\n %d ->%s left chat deleting from list",ts_fd,uname);  
           sprintf(buffer,"\n %s has left the chat",uname);  
           addr a = h ;  
           do{  
                a = a->next;  
                sfd = a->port;  
