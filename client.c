#include <stdio.h>  
 ...
 int main(int argc, char *argv[]){  
  pthread_t thr1,thr2;          // variable to hold thread ID  
     if( argc != 2 ){       
      ...
      bzero((char *) &serv_addr, sizeof(serv_addr));  //provide info
      serv_addr.sin_family = AF_INET;  
      serv_addr.sin_port = htons(VINPORT);  
      serv_addr.sin_addr.s_addr = inet_addr(argv[1]);  
      
      bzero(buf,10);  			//Username Entry
      printf("\nENTER YOUR NAME::");  
      fgets(buf,10,stdin);  
      __fpurge(stdin);       
      buf[strlen(buf)-1]=':';  
    
      if(connect(sockfd, (struct sockaddr *) &serv_addr, sizeof(serv_addr))==-1)  //client connection to server
      {  
        printf("client connect error\n");  
        exit(0);  
      }  
      else  
        printf("\n%s connected to server",buf);  
      printf("\nYOU JOINED AS- %s",buffer-1);  
 ...
 void* chat_read(int sockfd)  //reading continuosly from socket
 {  
 ...
 void* chat_write(int sockfd)  //writing continuously to socket
 {  
      while(1)  
      {  
        printf("%s",buf);  
         ...
         if(strlen(buffer)-1>sizeof(buffer)){  
             printf("buffer size fullt enter within %d characters\n",sizeof(buffer));  
             bzero(buffer,MAXDATALEN);  
             __fpurge(stdin);  
             }  
